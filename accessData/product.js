  ...
  function convertSort(sortColumn) {
    if (typeof sortColumn !== 'object') {
        ...
        sortColumn.split(',').forEach(element => {
            ...
            sortColumn = sort;
            console.log('sortColumn 1', sortColumn)
        });
    } else {
...
/////////////////////////////////////////////
//categoryID
productRouter.post('/products', func.getSiteID, (req, res) => { // func.checkAuthenticated
    // check('userId').not().isEmpty().isString();
    check('siteID').not().isEmpty().isString();
    // check('authLevel').not().isEmpty().isString().isLength({ min: 2, max: 3 });
    sanitizeBody('notifyOnReply').toBoolean()
    ...
    if (!errors.isEmpty()) {
        ...
        console.log(by, sort, skip, perPage)
        Products.find(by)
        .sort(sort)
        .skip(skip)
        .limit(perPage)
...
productRouter.get('/geteditlevel', func.getSiteID, (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            Site.findById(req.siteID).exec().then( (results)=> {
                if (results) {
...
productRouter.post('/createproduct', func.checkAuthenticated, async (req, res) => { // SPOTTODO Only if it`s Admin or Manager
    check('userId').not().isEmpty().isString();
    check('siteID').not().isEmpty().isString();
